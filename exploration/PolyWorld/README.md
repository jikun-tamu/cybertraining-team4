<!-- PROJECT USAGE NOTES (added during cleanup 2026-02) -->
## How this folder is used in the project

PolyWorld is evaluated as an alternative building-polygon extractor
(CVPR 2022, GNN-based) compared against SAM3_Claude outputs.

**Structure after reorganization:**
```
prediction.py               ← batch CrowdAI inference (main script)
predict_single_image.py     ← single-image inference (debug/demo)
predict_tiled_polyworld.py  ← tiled inference for large images (key script)
coco_to_shp.py              ← COCO JSON → shapefile (for QGIS)
coco_IoU_cIoU.py            ← IoU / C-IoU evaluation metrics
dataloader_crowdai.py       ← CrowdAI PyTorch Dataset
utils.py                    ← Hungarian matching + polygon conversion helpers
models/                     ← R2U_Net backbone + GNN matching architectures
trained_weights/            ← pretrained weights (backbone 150 MB, matching 1.4 MB)
notebooks/                  ← exploratory notebooks (Xihan, 2024-12-11)
sample_test/                ← test outputs (JSON predictions + overlay PNGs)
archive/                    ← Untitled-2.ipynb (incomplete, kept for safety)
```

**Quick test (single image):**
```bash
cd /media/gisense/xihan/250812_tamu_cybertraining_team4/PolyWorld
python predict_single_image.py
# output: sample_test/predictions.json + sample_test/overlay.png
```

**Tiled inference on a large image:**
```bash
python predict_tiled_polyworld.py
# output: sample_test/tiled_predictions_v8.json + sample_test/overlay_tiled_v8.png
```
